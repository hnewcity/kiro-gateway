...
class TestCallKiroMCPAPI:
    ...
    async def test_mcp_request_includes_profile_arn_for_enterprise_ide(self, mock_auth_manager):
        """
        What it does: Verifies Enterprise Kiro IDE OIDC accounts add profileArn.
        Purpose: Enterprise MCP web_search calls require top-level profileArn.
        """
        print("Setup: Switching auth manager to Enterprise Kiro IDE OIDC...")
        from kiro.auth import AuthType

        mock_auth_manager._auth_type = AuthType.AWS_SSO_OIDC
        mock_auth_manager._client_id_hash = "abc123def456"
        assert mock_auth_manager.profile_arn
        ...
        print("Checking: profileArn is present at top level...")
        sent_body = mock_post.await_args.kwargs["json"]
        assert sent_body.get("profileArn") == mock_auth_manager.profile_arn
        assert "profileArn" not in sent_body.get("params", {})
