...
class AccountManager:
    ...
    async def _initialize_account(self, account_id: str) -> bool:
        ...
        try:
            ...
            # Fetch the dynamic model list from the Q control plane.
            # runtime.kiro.dev does not serve ListAvailableModels (returns
            # UnknownOperationException), so management_host (q.{region}.amazonaws.com)
            # is used here regardless of the inference endpoint. Falls back to the
            # static list if the request fails or returns nothing.
            params = {"origin": "AI_EDITOR"}
            if auth_manager.profile_arn:
                params["profileArn"] = auth_manager.profile_arn

            list_models_url = f"{auth_manager.management_host}/ListAvailableModels"
            ...
            try:
                ...
                if response.status_code == 200:
                    data = response.json()
                    models_list = data.get("models", []) or FALLBACK_MODELS
                else:
    ...
    async def _refresh_account_models(self, account_id: str) -> None:
        ...
        # Attempt to fetch the dynamic model list from the Q control plane.
        # ListAvailableModels lives on management_host (q.{region}.amazonaws.com),
        # not runtime.kiro.dev. On failure the existing cache is kept.
        ...
        try:
            ...
            list_models_url = f"{account.auth_manager.management_host}/ListAvailableModels"
