...
def _strip_billing_attribution(text: str) -> str:
    ...
    to the system prompt content. In sdk-cli mode the ``cch`` segment is a fresh
    random hex per request; in CLI / VSCode mode the line is stable but still
    carries the per-version ``cc_version`` and per-entrypoint ``cc_entrypoint``.
    Either way the line changes across versions / entrypoints and defeats any
    prompt cache keyed on the prompt prefix on upstreams that do not understand
    ...
    Args:
        text: A system prompt string (or a single system text block).

    Returns:
        The input with a leading attribution line stripped, or unchanged if
        stripping is disabled or no attribution line is present.
    """
...
def extract_system_prompt(system: Any) -> str:
    ...
    system text block (``x-anthropic-billing-header: ...``). The line changes
    across versions / entrypoints (and is fully random in sdk-cli mode) and
    would invalidate any upstream prompt cache keyed on the prompt prefix, so we
    drop attribution blocks (and any attribution prefix on string-form prompts)
    here.
